...
PX4/MAVLink 기반 드론의 사이버-물리 보안 연구를 위한 **다채널 시간 정렬 공개 데이터셋**이다. Pixhawk 6C Mini 비행 제어기와 Raspberry Pi 4 온보드 컴퓨터, Gazebo HITL 시뮬레이터를 사용하여 동일 비행을 4가지 서로 다른 관측 채널에서 동시 캡쳐하였다. 도메인 간 비교를 위해 검증된 실기체 정상 비행 자료(2 sessions)도 함께 제공한다.
